...
const JardineroPanel = () => {
  ...
  const cargarTareas = async () => {
    try {
      ...
      const respuesta = await fetch(`https://proyecto-production-600d.up.railway.app/tareas/jardinero/${idJardinero}`); // URL actualizada
  ...
  const completarTarea = async (id) => {
    try {
      const respuesta = await fetch(`https://proyecto-production-600d.up.railway.app/tareas/${id}/completar`, { // URL actualizada
        method: 'PUT'
      });
      const datos = await respuesta.json();
      if (!respuesta.ok) {
